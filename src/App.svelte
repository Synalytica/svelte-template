...
<ul class="navigation-links">
  ...
  <li>
    <a
      href="/hello/svelte"
      use:link
      use:active={{ path: '/hello/*', className: 'active' }}>
      Say hi!
    </a>
  </li>
  <li>
...
<p class="navigation-buttons">
  <button on:click={() => push('/wild/something')}>
    Visit /wild/something
  </button>
  <button on:click={() => pop()}>Go back</button>
  <button on:click={() => replace('/wild/replaced')}>
...
<ul class="navigation-dynamic-links">
  {#each dynamicLinks as dl (dl.id)}
    <li>
      <a id="dynamic-link-{dl.id}" href={dl.link} use:link use:active>
        Dynamic Link {dl.id}
      </a>
      -
      <i
        id="delete-link-{dl.id}"
        on:click={() => (dynamicLinks = dynamicLinks.filter(e => e.id != dl.id))}>
        delete link
      </i>
    </li>
  {/each}
